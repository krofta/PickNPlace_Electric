...
void print_grauwert_menu()
{
	...
	printf("\n");
	printf(" __________________________________________________\n");
	printf("  Pfeiltaste RECHTS/LINKS = Auswahl                \n");
	printf("  Enter                   = Bestaetigen            \n");
	printf("  ESC                     = Zurueck                \n");
	printf("  F1                      = Info zur Funktion      \n");
	printf(" __________________________________________________\n");
}
...
void print_laplace_menu()
{
	...
	printf("\n");
	printf(" __________________________________________________\n");
	printf("  Pfeiltaste HOCH/RUNTER  = Auswahl                \n");
	printf("  Enter                   = Bestaetigen            \n");
	printf("  ESC                     = Zurueck                \n");
	printf("  F1                      = Info zur Funktion      \n");
	printf(" __________________________________________________\n");
...
void print_mittelwert_menu(int Art, int iDIMxy, int gew) {
	...
	printf(" __________________________________________________\n");
	printf("  Pfeiltaste HOCH/RUNTER  = Auswahl                \n");
	printf("  Pfeiltaste LINKS/RECHTS = Inkrement/Dekrement    \n");
	printf("  Enter                   = Ausfuehren             \n");
	printf("  ESC                     = Zurueck                \n");
	printf("  F1                      = Info zur Funktion      \n");
	printf(" __________________________________________________\n");
	char buf[3];
...
void print_menu_textur(int winkel, int graustufen)
{
	...
	printf(" __________________________________________________\n");
	char buf[5];
	setCursor(START_CURSOR_LINE + 2, 40);
	//itoa(winkel, buf, 10);
	sprintf(buf,"%i",winkel);
	fputs(buf, stdout);
	setCursor(START_CURSOR_LINE + 4, 40);
...
void menu_textur(unsigned char img[MAXXDIM][MAXYDIM], unsigned char img2[MAXXDIM][MAXYDIM])
{
	...
	while (wahl != KEY_ESC)
	{
		setCursor(cursorLine + START_CURSOR_LINE, 40);
...
void print_menu_segmentierung(int threshold, int intervall[3])
{
	cls();
	setColor(FOREGROUND_RED);
	printf(" __________________________________________________\n");
	...
	printf("  6. Blister Bericht  (SPM2 Bericht)           [ ]\n");
	printf("\n");
	printf(" __________________________________________________\n");
	printf("  Pfeiltaste HOCH/RUNTER  = Auswahl                \n");
	printf("  Pfeiltaste LINKS/RECHTS = Inkrement/Dekrement    \n");
	...
	printf(" __________________________________________________\n");
	char buf[5];
	setCursor(START_CURSOR_LINE, 48);
	//itoa(threshold, buf, 10);
	sprintf(buf,"%i",threshold);
	fputs(buf, stdout);
	for (int i = 0; i < 3; i++) {
		setCursor(START_CURSOR_LINE + i + 2, 48);
...
void menu_segmentierung_help(int HelpFunc)
{
	...
	case 4:
		...
		break;
	case 5:
		printf("  6. Blister Bericht  \n");
		printf("  Klassifiziert die Objekte in einem Blister.\n");
		printf("  Das Ergebnis wird als Bericht unter c:/bv/bericht.txt gespeichert.\n");
		break;
	default:
		return;
	}
...
void menu_segmentierung(unsigned char img[MAXXDIM][MAXYDIM], unsigned char img2[MAXXDIM][MAXYDIM])
{
	...
	char buf[5];
	print_menu_segmentierung(threshold, intervall);
	while (wahl != KEY_ESC)
	{
